...
directory = r"wordle_clone\resources\data\\"

def sort_word(word,val):

    if val > 70: #easy word
        with open(file_name + "easy_letter words.txt", "a") as fl_easy:
            fl_easy.write(word)

    if 50 < val < 70: #medium word
        with open(file_name + "medium_letter words.txt", "a") as fl_medium:
            fl_medium.write(word)

    if val < 50: #hard word
        with open(file_name + "hard_letter words.txt", "a") as fl_hard:
            fl_hard.write(word)
...
for no_of_ltr in range(5,8):
    ...
    file_name = directory+str(no_of_ltr)
